...
fn main() -> ! {
    ...
    channel2
        .input_pin(&board.microphone_pins.mic_in.degrade())
        .enable_interrupt();
    channel2.reset_events();

    let channel3 = gpiote.channel3();
    channel3
        .input_pin(&board.pins.p1_04.into_floating_input().degrade())
        .hi_to_lo()
        .enable_interrupt();
...
/// Interrupt handler that deals with adding events to the EVENT_QUEUE
#[interrupt]
fn GPIOTE() {
    ...
    cortex_m::interrupt::free(|cs| {
        if let Some(gpiote) = GPIO.borrow(cs).borrow().as_ref() {
            ...
            cortex_m::interrupt::free(|cs| {
                if let Some(event) = EVENT_QUEUE.borrow(cs).borrow_mut().as_mut() {
                    if button_a_pressed {
